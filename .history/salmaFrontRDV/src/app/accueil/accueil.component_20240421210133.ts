...
export class AccueilComponent implements OnInit {
    ...
    this.patientService.getAllMessages()
      .subscribe(
        (messages: MessagePatient[]) => {
          ...
          this.showResponseDetails = new Array(messages.length).fill(false);
        },
        (error: any) => {
  ...
  toggleResponseDetails(index: number, event: any): void { // Prendre l'index comme argument
    this.showResponseDetails[index] = event.target.checked; // Utiliser l'index pour accéder à l'élément correspondant dans le tableau
  }
