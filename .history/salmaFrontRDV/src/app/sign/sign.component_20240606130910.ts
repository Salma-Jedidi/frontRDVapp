 ...
 */
  ...
  login(loginForm: NgForm): void {
    ...
    if (this.webcamImage && this.user.email && this.user.password) {
        // Préparer la requête pour la reconnaissance faciale
        const blob = this.dataURItoBlob(this.webcamImage.imageAsDataUrl);
        const formData = new FormData();
        formData.append('file', blob, 'webcamImage.jpg');
        formData.append('email', this.user.email);

        // Préparer la requête pour l'authentification par email/mot de passe
        const credentials = {
            email: this.user.email,
            password: this.user.password
        };

        // Appeler les deux services en parallèle
        forkJoin({
            faceAuth: this.http.post('http://127.0.0.1:5000/login', formData).pipe(catchError(error => of(null))),
            credentialsAuth: this.authService.signin(credentials).pipe(catchError(error => of(null)))
        }).subscribe(({ faceAuth, credentialsAuth }) => {
            if (faceAuth) {
                console.log('Login success with image', faceAuth);
                this.handleLoginSuccess(faceAuth);
            }

            if (credentialsAuth) {
                console.log('Login success with email/password', credentialsAuth);
                this.handleLoginSuccess(credentialsAuth);
            }

            if (!faceAuth && !credentialsAuth) {
                this.errorMessage = 'Login failed with both methods';
            }
        });
    } else {
        console.error('Email, password, and webcam image are required for login.');
        this.errorMessage = 'Email, password, and webcam image are required for login.';
    }
