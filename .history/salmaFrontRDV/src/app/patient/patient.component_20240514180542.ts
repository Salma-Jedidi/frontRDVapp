...
addRDVAfterAvailabilityCheck(): void {
  this.adminService.addRDV(this.rdv).subscribe(
    (addedRDV: RDV) => {
      ...
      this.adminService.assignPatientAndMedecinToRD(this.rdv.nomDuPatient, this.rdv.nomDuMedecin, addedRDV).subscribe(
        () => {
...
showAvailableTimes(): void {
  this.adminService.suggestAvailableTimes(this.rdv.nomDuMedecin, this.rdv.dateRDV, 8)
    .subscribe(
      (availableTimes: LocalTime[]) => {
        console.log('Available times:', availableTimes);
        // Afficher les temps disponibles dans votre interface utilisateur
      },
      (error) => {
