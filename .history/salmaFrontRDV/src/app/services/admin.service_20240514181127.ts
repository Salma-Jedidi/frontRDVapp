...
export class AdminService {
  ...
  checkAvailability(nomDuMedecin: string, dateRDV: undefined, heureRDV: string): Observable<boolean> {
    const url = `${this.apiUrl}/checkAvailability?nomDuMedecin=${nomDuMedecin}&dateRDV=${dateRDV}&heureRDV=${heureRDV}`;
    return this.http.get<boolean>(url);
  }

  suggestAvailableTimes(nomDuMedecin: string, dateRDV: undefined, numberOfSuggestions: number): Observable<string[]> {
