...
export class PatientService {
  ...
  sendMessage(cinPatient: number, contenue: string, messagePatient: MessagePatient, httpOptions: any): Observable<MessagePatient> {
    // Effectuer la requête HTTP POST
    return this.http.post<MessagePatient>(`${this.apiUrl}/messagePatient/${cinPatient}?contenue=${contenue}`, messagePatient, httpOptions)
      .pipe(
        map((response: HttpEvent<MessagePatient>) => {
          if (response instanceof HttpResponse) {
            return response.body; // Retourne le corps de la réponse HTTP si la requête a été exécutée avec succès
          } else {
