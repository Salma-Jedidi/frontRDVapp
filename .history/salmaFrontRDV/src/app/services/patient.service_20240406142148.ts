...
export class PatientService {
  ...
  sendMessage(nomPatient: string, contenueMessage: string, message: Messages): Observable<Messages> {
    return this.http.post<Messages>(`${this.apiUrl}/patients/${nomPatient}/contenue-message=${contenueMessage}/message`, message);
  }
  choisirModePaiement(cin: number, modePaiement: string, typeCaisse: string): Observable<string> {
    const url = `${this.apiUrl}/${cin}/mode-paiement/type-caisse?modePaiementChoisi=${modePaiement}&typeCaisse=${typeCaisse}`;
    return this.http.post<string>(url, null);
