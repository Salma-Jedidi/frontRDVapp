...
export class PatientService {
  ...
  sendMessage(cinPatient: number, contenue: string, message: MessagePatient): Observable<MessagePatient> {
    // Créez les en-têtes nécessaires
    const headers = new HttpHeaders();
  
    // Ajoutez les en-têtes requis, si nécessaire
    // headers = headers.set('Authorization', 'Bearer ' + authToken); // Exemple: Ajout d'un jeton d'authentification
  
    // Effectuez la requête HTTP POST
    return this.http.post<MessagePatient>(`${this.apiUrl}/messagePatient/${cinPatient}?contenue=${contenue}`, message, { headers: headers });
  }
