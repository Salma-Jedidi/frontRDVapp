...
export class AdminService {
  ...
  checkAvailability(nomDuMedecin: string, dateRDV: string, heureRDV: string): Observable<boolean> {
    const url = `${this.baseUrl}/checkAvailability?nomDuMedecin=${nomDuMedecin}&dateRDV=${dateRDV}&heureRDV=${heureRDV}`;
    return this.http.get<boolean>(url);
  }

  suggestAvailableTimes(nomDuMedecin: string, dateRDV: string, numberOfSuggestions: number): Observable<LocalTime[]> {
    const url = `${this.baseUrl}/suggestTimes?nomDuMedecin=${nomDuMedecin}&dateRDV=${dateRDV}&numberOfSuggestions=${numberOfSuggestions}`;
    return this.http.get<LocalTime[]>(url);
  }
