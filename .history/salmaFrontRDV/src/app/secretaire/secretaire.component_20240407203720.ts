...
utiliserPourcentageRDVPayes(): Observable<number> {
  return this.patientService.getPourcentageRDVPayes();
}

pourcentageRDVPayes(): void {
  this.utiliserPourcentageRDVPayes().subscribe((pourcentage: number) => {
    console.log('Pourcentage récupéré depuis le backend :', pourcentage);
    // Maintenant, vous pouvez utiliser cette valeur où vous le souhaitez dans votre code.
  });
}
ngAfterViewInit(): void {
  ...
  this.pourcentageRDVPayes().subscribe((pourcentage: number) => { // Création du pie chart
