...
export class SecretaireComponent implements OnInit{
  ...
  barChart: Chart<"bar", number[], string> | null;
  patient:Patient={
      idPatient: 0,
      nomPatient: '',
...
initializePieChart(): void {
  ...
  this.pieChart = new Chart(pieChartContext, {
    type: 'pie',
    data: {
      labels: ['RDV Payés', 'non payés'],
      datasets: [
        {
          data: [0, 100], // Valeurs initiales
      ...
      datasets: [
        {
          ...
          data: [0, 0, 0, 0], // Valeurs initiales
...
updatePieChart(pourcentage: number): void {
  this.pieChart.data.datasets[0].data = [pourcentage, 100 - pourcentage];
  this.pieChart.update();
}

updateBarChart(nombreRDVAcceptes: number, nombreRDVAnnules: number, nombreRDVEnInstant: number, nombreRDVModifies: number): void {
  this.barChart.data.datasets[0].data = [nombreRDVAcceptes, nombreRDVAnnules, nombreRDVEnInstant, nombreRDVModifies];
