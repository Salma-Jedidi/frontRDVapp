...
pourcentageRDV(): Observable<number> {
  // Appel à votre service pour récupérer le pourcentage depuis le backend
  return this.patientService.getPourcentageRDVPayes();
}
pourcentageRDVPayes():void {
  this.patientService.getPourcentageRDVPayes().subscribe((pourcentage: number) => {
    console.log('Pourcentage récupéré depuis le backend :', pourcentage);
    // Vous pouvez maintenant utiliser cette valeur où vous le souhaitez dans votre code.
  });
}
