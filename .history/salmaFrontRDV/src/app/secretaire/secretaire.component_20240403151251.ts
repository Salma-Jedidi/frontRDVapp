...
export class SecretaireComponent implements OnInit{
  ...
  associerDossierMedicalAuPatient(cin: number) {
    this.patientService.associerDossierMedicalAuPatient(cin, this.dossierMedical)
      .subscribe(
        (response) => {
          console.log('Dossier médical associé avec succès au patient:', response);
          // Réinitialiser l'objet dossierMedical après l'association réussie
