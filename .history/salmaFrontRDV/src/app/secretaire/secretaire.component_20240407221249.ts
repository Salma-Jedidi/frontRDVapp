...
this.nombreRDVAcceptes$ = this.patientService.getNombreRDVByEtat('ACCEPTER');
this.nombreRDVAnnules$ = this.patientService.getNombreRDVByEtat('ANNULER');
this.nombreRDVEnInstant$ = this.patientService.getNombreRDVByEtat('ENINSTANT');
this.nombreRDVModifies$ = this.patientService.getNombreRDVByEtat('MODIFIER');
...
ngAfterViewInit(): void {
  this.createPieChart();
  forkJoin([
    this.nombreRDVAcceptes$,
    this.nombreRDVAnnules$,
    this.nombreRDVEnInstant$,
    this.nombreRDVModifies$
  ]).subscribe(([acceptes, annules, enInstant, modifies]) => {
    // Une fois que toutes les données sont disponibles, appelez createBarChart()
    this.createBarChart(acceptes, annules, enInstant, modifies);
  });
}
createPieChart(): void {
